...
\usepackage{subcaption, empheq, setspace, lscape, dsfont}
...
\begin{figure}[h]
\centerline{\includegraphics[width=1.0\linewidth]{linear-sing-pert-mm-ii}}
...
\begin{figure}[ht!]
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{keff-small-new}
    \subcaption{ \label{fig:abc-keff-1}}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{keff-large-new}
...
Fig. (\ref{fig:abc-keff-1}) we find that the eigenvectors $v_2$ and
$v_3$ provide the desired coordinates on the nonlinear surface. This
is illustrated in Fig. (\ref{fig:abc-dmaps}) in which the surface is
...
\begin{figure}[ht!]
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{dmaps-2d-phi1}
    \subcaption{ \label{fig:abc-dmaps-1}}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \centering
    \includegraphics[width=\textwidth]{dmaps-2d-phi2}
...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.49\textwidth]{dmaps-3d-keff-phi1}
...
=
 P
\,
 \mathrm{diag}(\mathrm{e}^{\lambda_- t},\mathrm{e}^{\lambda_+ t},1)
\,
 P^{-1}
\,
\left[\begin{array}{c}
...
\end{array}\right]
&=&
 \frac{\lambda_+ A_0 + (\lambda_+ + k_2)B_0}{\Delta \lambda}
\left[\begin{array}{ccc}
 -(1+\lambda_-/k_2) \\ \lambda_-/k_2 \\ 1
\end{array}\right]
 \mathrm{e}^{\lambda_- t}
\\
&{}&
...
\begin{align}
   V_0 + V_+\,\mathrm{e}^{\lambda_+ t} + V_-\,\mathrm{e}^{\lambda_- t}
  =
  & V_0\\
  + & V_+\,\mathrm{e}^{\lambda_+ t} \otimes \mathds{1} \\
  + & (V_- \oslash V_+)\,\mathrm{e}^{-\Delta \lambda t} \\
\end{align}
...
setting, our data-collection times might start at
$t_1 = 1/(2 |\lambda_+|)$ at which point the slow exponential has
already decayed by $\approx 40\%$. A separation of timescales implies
that the fast component is effectively extinguished by this first time
$t_1$. When we only observe $C(t)$, this leads to

\begin{align}
 \left\vert \frac{\lambda_+ A_0 + (\lambda_++k_2)B_0}{\lambda_- A_0 + (\lambda_-+k_2)B_0} \right\vert 
\,
 \mathrm{e}^{-\Delta \lambda t_1}
\ll
 1
\end{align}
