...
def plot_data(x, y, z, color, label, **kwargs):
    fig = plt.figure()
    ...
    ax.set_zlabel(r'$k_2$')
    ax.grid(False)
    s = ax.scatter(x, y, z, c=color, **kwargs)
    ax.grid(False)
    cb = fig.colorbar(s, ticks=np.linspace(np.amin(color), np.amax(color), 7))
    cb.set_label(label)
    # if cbnorm is not None:
